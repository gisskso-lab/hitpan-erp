...
begin
  ...
  try
    ...
    BatchContent.Add(Format('if !EXISTING_DATA! EQU 0 if !TBL_COUNT! GTR 0 if !TBL_COUNT! LSS 123 (echo 불완전 스키마 !TBL_COUNT!/123 감지 - 재생성. & mysql -u root -p%s -e "DROP DATABASE IF EXISTS %s; CREATE DATABASE %s CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci; GRANT ALL ON %s.* TO ''%s''@''localhost''; FLUSH PRIVILEGES;" & set TBL_COUNT=0)', [MariaRootPw, G_DbName, G_DbName, G_DbName, G_DbUser]));
    ...
    // 봉합 검증 가드(1.2.15, SHIP-DDL-01 정정 2026-06-23 / A안 고리2 2026-06-29 local_update_status 122→123): import 후 테이블 123개 + users 실존 재확인. 미달이면 명확한 실패(헌법 #15·#19).
    ...
    BatchContent.Add(Format('if !FINAL_COUNT! LSS 123 (echo [오류] DB 초기 설정 실패 - 테이블 !FINAL_COUNT!/123개만 생성됨. & exit /b 1) else (echo DB 스키마 검증 완료 - 테이블 !FINAL_COUNT!개 + users 정상.)', []));
