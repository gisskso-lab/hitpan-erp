// v3-B3: v3b2 + 로딩 대기 증가 (Blazor WASM 첫 부팅 4초 + 페이지별 3.5초)
...
(async () => {
    console.log('B3. v3b2 + 로딩 대기 증가 (부팅 4s + 페이지 3.5s)');
    ...
    console.log('');
    console.log(`=== B3 결과: ${pass}/${results.length} PASS ===`);

    const fs = require('fs'), path = require('path');
    const dir = path.join(__dirname, '..', '..', 'tests', 'scenarios', 'reports');
    if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
    const stamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
    const out = path.join(dir, `audit-erp-99-v3b3-${stamp}.json`);
