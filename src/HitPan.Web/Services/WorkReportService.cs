...
public sealed class WorkReportService(HttpClient http, ILogger<WorkReportService> logger)
{
    ...
    /// </returns>
    public async Task<(bool Ok, string? Message, string? ReportId)> SaveAsync(
        string? id, SaveWorkReportModel model, CancellationToken ct = default)
    {
        try
        {
            ...
            using (res)
            {
                ...
                if (string.IsNullOrWhiteSpace(id))
                {
                    var created = await res.Content
                        .ReadFromJsonAsync<CreatedReportResponse>(cancellationToken: ct)
                        .ConfigureAwait(false);
                    return (true, null, created?.ReportId);
                }
        ...
        catch (Exception ex)
        {
            ...
            return (false, "저장하지 못했습니다. 잠시 후 다시 시도해 주세요.", null);
        }
    }
    ...
    public async Task<(bool Ok, string? Message)> SubmitAsync(string id, CancellationToken ct = default)
    {
        ...
        catch (Exception ex)
        {
            ...
            return (false, "결재에 올리지 못했습니다. 잠시 후 다시 시도해 주세요.");
        }
    }
    ...
    private sealed class CreatedReportResponse
    {
