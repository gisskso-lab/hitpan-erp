...
public class AuthController : ControllerBase
{
    ...
    public async Task<IActionResult> Login([FromBody] LoginRequest request, CancellationToken ct)
    {
        try
        {
            ...
            //   여기 있던 loopback 판정(isMainPcLoopback)은 **고객사에서 항상 참**이었다.
            //   터널이 고객 PC 안에서 localhost 를 다시 부르고, 히트판은 127.0.0.1 에만 귀를 열며,
            //   사내 PC 경로는 Host 를 버리고 X-Forwarded-For 를 안 붙인다.
            //   ⇒ 그 위에 세운 것(한도 면제·폐기 부활·이름 표식)이 전부 전 기기에 적용돼
            //     요금정책과 접근 차단이 함께 무너질 뻔했다.
            //   근거: docs/검증/병렬이슈/20260810_병렬이슈_09_작2봉합_독립반증.md
            //
            //   ⭐ 메인PC 구분은 tenant_devices.is_main_pc(DB-86)에서 하고,
            //     MainPcRegistrationService 가 서버 기동 시점에 표시한다(20260810작3).
            //     "요청이 어디서 왔나" 가 아니라 "내가 어디서 도나" 를 보므로 흔들리지 않는다.
            //
            //   ⚠️ 이 자리에 접속 경로 판정을 **다시 넣지 말 것.** 두 번 무너진 자리다.
            //
            ...
            if (!string.IsNullOrEmpty(response.TenantId) && !string.IsNullOrEmpty(request.DeviceFingerprint))
            {
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(userId))
                    {
                        ...
                        // 🔴 2026-08-10 [3-V] 철회 — loopback 으로 메인PC 를 판정하지 않는다.
                        //
                        //   종전 코드는 isMainPcLoopback 이면 이름에 "(자료 보관 컴퓨터)" 를 붙였다.
                        //   그런데 그 판정은 고객사에서 **항상 참**이다:
                        //     · 터널(cloudflared)은 고객 PC 안에서 돌며 자기 PC 의 localhost 를 다시 부른다
                        //       (HitPan-Universal.iss:1134 — ingress origin = http://localhost:5257)
                        //     · 히트판은 --urls http://127.0.0.1 로 loopback 에만 귀를 연다(:2359)
                        //       ⇒ 도달한 요청은 100% loopback
                        //     · 사내 다른 PC 경로(web-server.ps1:98)는 Host 를 버리고 X-Forwarded-For 를
                        //       붙이지 않는다 ⇒ 프록시 헤더로도 못 거른다
                        //   ⇒ **전 기기에 표식이 붙어** 구분하려고 만든 표식이 구분을 없앤다.
                        //   근거: docs/검증/병렬이슈/20260810_병렬이슈_09_작2봉합_독립반증.md (V9-3)
                        //
    ...
    // 🔴 BuildMainPcName 제거 (2026-08-10 [3-V]).
    //   메인PC 이름에 표식을 붙이던 함수다. 함수 자체는 정확했으나(100자 계산·중복 부착 방지),
    //   그것을 부르는 판정(loopback)이 고객사에서 항상 참이라 **전 기기에 표식이 붙었다.**
    //   구분하려고 만든 표식이 구분을 없앴다.
    //   ⇒ 메인PC 이름은 이제 MainPcRegistrationService 가 등록 시점에 직접 정한다(20260810작3).
    //     이름을 만드는 자리를 두 곳에 두지 않는다 — 갈라지면 어느 쪽이 진짜인지 알 수 없게 된다.
}
